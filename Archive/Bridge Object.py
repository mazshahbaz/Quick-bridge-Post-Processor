        ...
        def CreateSpans():
            ...
            **Note: Had to break into two loops, Spans would not continue
            ...
            for span_label in span_list:
                spans[span_label] = Span(span_label)
            return spans
        
        self.spans = CreateSpans()
        ...
        def create_girders():
            ...
            **Note: Had to break into two loops, Spans would not continue
            ...
            for girder_label in girder_list:
                girders[girder_label] = Girder(span_label, girder_label)
            return girders         
    
        self.girders = create_girders()
...
class Girder(Span):
    ...
    def __init__(self,span_label, girder_label):
#        Span.__init__(self, span_label)
        ...
        def create_load_cases(stations):
            load_case_labels = []
            load_cases = {}
            ...
            for load_case_label in load_case_labels:
                ...
                load_cases[load_case_label[0]] = LoadCase(span_label, girder_label, stations, load_case_label[0], load_case_label[1])
            return load_cases
        
        self.load_cases = create_load_cases(self.stations)
...
class LoadCase(Girder):
    def __init__(self, span_label, girder_label, stations, load_case_label, step_type):
        self.span_label = span_label
        self.girder_label = girder_label
...
#        self.force_fields = [M3_field, V2_field, M2_field, V3_field, P_field, T_field]
        ...
        def add_forces(forces, span_label, girder_label, load_case_label):
            for row in raw_data:
        ...
        self.forces = add_forces(self.forces, self.span_label, self.girder_label, self.load_case_label)
    ...
    """ Get indiividual vlaues of a force at a cut location"""
