...
class GlobalBridge(object):
    """
    This Class creates a workbook that represents girders along the entire bridge length, not just span.
    **Currently only works for bridges that have the same number of girders at each span
    ...
    def __init__(self, Bridge):
        ...
        """
        Find all girders with the same name in each span and put them in a list which represents a "Global Girder"
        organize each global girder by label in a dictionary with the girer label as the key
        """
        self.global_girders = {}
...
class GlobalGirder(GlobalBridge):
    """
    Creates a single global_girder object out of multiple girders.
    Combine the girder tables in each girder in the global_girder list to form a SINGLE table for each force type.
    """
...
#        print("creating global firder for: ", self.global_girder_label)
        ...
        print(M3_tables)
        local_table_labels = M3_tables[0][0]
...
#                print("appending table")
        ...
        self.global_tables = {"M3": self.global_M3_table, "V2": self.global_V2_table, "M2": self.global_M2_table, "V3": self.global_V3_table, "P": self.global_P_table, "T_table": self.global_T_table}
        self.global_table_keys = [key for key in self.global_tables.keys()]
