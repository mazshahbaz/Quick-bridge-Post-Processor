...
class Girder(Span):
    ...
    def __init__(self,span_label, girder_label, raw_data, global_girder = False):
#        Span.__init__(self, span_label)
        ...
        def create_stations(raw_data):
            ...
            for row in raw_data:
                ...
                if global_girder:
                    condition = (row[girder_field] == girder_label) and cut_label not in stations.keys()
                else:
                    condition = (row[span_field] == span_label) and (row[girder_field] == girder_label) and cut_label not in stations.keys()
                if condition:
...
class LoadCase(Girder):
    ...
    """
    def __init__(self, span_label, girder_label, stations, load_case_label, step_type, raw_data, global_girder=False):
        self.span_label = span_label
        self.girder_label = girder_label
...
#        self.force_fields = [M3_field, V2_field, M2_field, V3_field, P_field, T_field]
        ...
        def add_forces(forces, span_label, girder_label, load_case_label, raw_data, global_girder):
            for row in raw_data:
                if global_girder:
                    condition = (row[span_field] == span_label) and (row[girder_field] == girder_label) and (row[load_case_field] == load_case_label) and (row[step_type_field] == step_type)
                else:
                    condition = (row[girder_field] == girder_label) and (row[load_case_field] == load_case_label) and (row[step_type_field] == step_type)
                if condition:
                    cut = row[cut_field]
...
class GlobalBridge(BridgeObject):
    def __init__(self, spans):
        self.spans = spans
        self.global_girder_labels = self.spans["Span1"].get_girder_labels()
        ...
        for global_girder_label in self.global_girder_labels:
            ...
            self.global_girders[global_girder_label] = GlobalGirder(global_girder)
            
class GlobalGirder(GlobalBridge):
    def __init__(self, global_girder):
        self.global_girder = global_girder
        self.global_labels
        
        
        for girder in global_girder:
            self.global_M3_table = 1
